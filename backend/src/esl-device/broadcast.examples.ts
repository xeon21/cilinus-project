...
export class BroadcastExamples {
   ...
   */
  broadcastToRoomExceptSender(socket: Socket, roomName: string, eventName: string, data: any) {
   ...
   */
  async broadcastToFiltered(filterFn: (socket: Socket) => boolean, eventName: string, data: any) {
   ...
   */
  notifyDeviceStatusChange(deviceId: string, status: 'online' | 'offline' | 'warning') {
    const notification = {
    ...
    // 해당 매장의 관리자에게만 알림
    this.server.to(`store-${deviceId.split('-')[1]}`).emit('device-status-update', notification);
   ...
   */
  async broadcastLargeData(eventName: string, largeData: any[], chunkSize: number = 100) {
...
export class EslDeviceBroadcastService {
  ...
  startSystemStatusBroadcast() {
    setInterval(() => {
      ...
      // 대시보드 구독자들에게만 전송
      this.server.to('dashboard-subscribers').emit('system-status', systemStatus);
    }, 5000); // 5초마다
  }
   ...
   */
  async broadcastToDevicesByLocation(location: string, eventName: string, data: any) {
    const sockets = await this.server.fetchSockets();
