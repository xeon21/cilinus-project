import {
  ResponseDto_UserData,
  ResponseDto_TreeDataAll,
  ResponseDto_GetMyItem,
  ResponseDto_GetMyNurseryData,
} from '../dto/response.dto'; // 게스트 로그인 응답
import {
  Controller,
  Param,
  Body,
...
export class UserInfoController {
  ...
  @ApiResponse({
    status: 401,
    description: '인증되지 않았거나 비밀번호가 틀렸습니다.',
  })
  ...
  @ApiOperation({
    ...
    summary: '사용자 정보 조회 ',
    description:
      '서버로 부터 받은 인덱스 번호를  사용하여 유저 기본 정보를 서버로 부터 조회합니다. ',
  })
  @ApiParam({
    //경로 파라미터 'uuid'에 대한 설명
    name: 'userIndex',
    required: true,
  ...
  @ApiOperation({
    summary: '내 나무 정보 조회 ',
    description:
      '서버로 부터 받은 인덱스 번호를  사용하여 내 나무 정보를 조회합니다다.',
  })
  @ApiParam({
    //경로 파라미터 'uuid'에 대한 설명
    name: 'userIndex',
  ...
  @ApiOperation({
    summary: '내 양묘장 정보 조회 ',
    description:
      '서버로 부터 받은 인덱스 번호를  사용하여 내 양묘장장 정보를 조회합니다다.',
  })
  @ApiParam({
    //경로 파라미터 'uuid'에 대한 설명
    name: 'userIndex',
  ...
  @ApiOperation({
    summary: '내 아이템 정보조회',
    description:
      '서버로 부터 받은 인덱스 번호를  사용하여 내 아이템템 정보를 조회합니다.',
  })
  @ApiParam({
    //경로 파라미터 'uuid'에 대한 설명
    name: 'userIndex',
