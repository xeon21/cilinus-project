...
md의 지침을 항상 따르세요. 제가 "실행"이라고 하면 CLAUDE.md에서 표시되지 않은 다음 테스트를 찾아 구현한 후, 해당 테스트를 통과할 만큼의 코드만 구현하세요.
...
빠른 속도보다 Slow & Deep Thinking 주목하세요. 
...
# API 추가작업
API 추가 작업이 있을 때 개발 모드에서는 받는 데이터와 보내는 데이터를 logger에 남기는 코드를 필수로 넣습니다. 
또한 Swagger 작업도 같이 병행하여 진행합니다.

#Todo 문서화 
프롬프트의 마지막이 '저장!'으로 끝난다면 아래와 같이 진행합니다. 
모든 작업 트랜잭션 전 Todo 목록을 파일로 저장합니다. 
파일은 프로젝트 root 경로에 있는 document/md_list폴더에 현재 시간+주작업폴더명을 파일명으로 하여 저장합니다. 
작업후에는 생성된 todo 리스트로 생성헌 md파일에 작업진행 결과를 덧붙여서 저장합니다. 
모든 작업 트랙잭션마다 파일을 따로 생성합니다. 
 
#DB Query 
모든 쿼리문은 파라미터가 적용된 완성된 쿼리문 형태로 logger에 필수로 남깁니다. 
예를 들어 이렇게 쿼리가 만들어진다면 const query = `UPDATE projects SET ${fields.join(', ')} WHERE id = ?`; 
모든 파라미터가 적용된 쿼리 스트링을 logger로 남깁니다. 
